...
\begin{eqnarray*}
\left( && \left(1 + \exp\left(- \left(\hat{\vartheta}_n - q_\alpha \text{diag}(\D_n)^{1/2}\right)\right)\right)^{-1}, \\
       && \left(1 + \exp\left(- \left(\hat{\vartheta}_n + q_\alpha \text{diag}(\D_n)^{1/2}\right)\right)\right)^{-1}\right).
\end{eqnarray*}
...
\item non-zero parameters in components of a mixture model
      (componentwise variable selection) and 
...
Stefan Haufe, Klaus-Robert M\"uller, Guido Nolte, and Nicole Kr\"amer (2009).
