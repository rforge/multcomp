...
setwd(tempdir())
download.file(dataurl, destfile = "chfls1.sav")

### data see http://www.spc.uchicago.edu/prc/chfls.php
chfls1 <- read.spss("chfls1.sav", to.data.frame = TRUE)
...
+ wealthdiffSD + Redu + Rhealth + Rhappy + Region, data=orgA, Hess=TRUE), trace = FALSE)
...
selection stops, when the AIC cannot be reduced further by removing a variable. 
...
s <- summary(glht(ordRegr, linfct = K, vcov = polrvcov))
...
hypotheses $K \beta = 0$ with the matrix $K$ being the $p \times p$ identity matrix. 
...
K <- multcomp:::mcp2matrix(ordRegr, linfct = mcp(Redu = 
c("univ - jcol = 0",
  "jcol - upmid = 0", 
  "upmid - lowmid = 0", 
  "lowmid - primary = 0", 
