...
mcp <- function(...) {
    ...
    if (is.null(names(linfct)))
        stop(sQuote("linfct"), " doesn't have a ", sQuote("names"), " attribute")

    types <- eval(formals(contrMat)$type)

    classes <- lapply(linfct, function(x) {
        if (is.matrix(x)) return("matrix")
        if (is.character(x)) {
            pm <- pmatch(x, types) 
            if (all(!is.na(pm))) return("character")
            if (all(is.na(pm))) return("expression")
            stop("Arguments ", sQuote(x[is.na(pm)]), " are not valid type arguments")
        }
        return(NA)
    })

    if (all(classes %in% c("matrix", "character")) || all(classes == "expression")) {
        class(linfct) <- "mcp"
        return(linfct)
    }
        ...
        if (is.character(K[[nm]])) {
            ...
            Kchr <- function(kch) {
                ...
                if (!is.na(pm)) {
                    ...
                    tmp <-  chr2matrix(kch, levels(mf[[nm]]))
                    tmpK <- tmp$K
                    m <- c(m, tmp$m)
