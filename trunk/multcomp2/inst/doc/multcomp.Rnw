...
could be some
kind of regression model where $Y = (y, x)$ can be split up into a dependent variable
...
\emph{general linear hypotheses} about the model parameters
...
observations $Y_1, \dots, Y_n$. Two situations
can be distinguished:
\begin{itemize}
  \item[A1:] the estimate follows a normal distribution with mean $\beta$ and covariance
                matrix $\Sigma$, i.e., $\hat{\beta} \sim \N(\beta, \Sigma)$.
  \item[A2:] the distribution of $\hat{\beta}$ is known only asymptotically:
                     $\hat{\beta} \rightarrow \N(\beta, \Sigma)$.
\end{itemize}
Moreover, we assume that an estimate $\V(\hat{\beta})$ of $\Sigma$ is available. It then holds that
$\K \hat{\beta} \sim \N(\K \beta, \K \Sigma \K^\top)$ (A1) or
$\K \hat{\beta} \rightarrow \N(\K \beta, \K \Sigma \K^\top)$ (A2), respectively.
...
of the form $z_j = (K_j \hat{\beta} -m_j) (K_j \V(\hat{\beta}) K_j^\top)^{-\frac{1}{2}}$
is the basis of all inference procedures.
The vector of standarized statistics
\begin{eqnarray*}
\z = (z_1, \dots, z_k) = \text{diag}\left(\K \V(\hat{\beta}) \K^\top\right)^{-\frac{1}{2}} (\K \hat{\beta} - \m)
\end{eqnarray*}
has a covariance matrix 
\begin{eqnarray*}
\V(\z) = \text{diag}\left(\K \V(\hat{\beta}) \K^\top\right)^{-\frac{1}{2}} 
         \K \V(\hat{\beta}) \K^\top 
         \left(\text{diag}\left(\K \V(\hat{\beta}) \K^\top\right)^{-\frac{1}{2}}\right)^\top.
\end{eqnarray*}
Under $H_0$ it holds that $\z \sim \T_{n - \text{Rank}(\K)}(0, \V(\z))$ (A1) or 
$\z \rightarrow \N(0, \V(\z))$ (A2), respectively.
