...
\emph{general linear hypotheses} \citep{Searle1971}, i.e, 
about linear functions of the vector of 
regression coefficients $\beta \in \R^p$:
\begin{eqnarray*}
H_0: \K \beta = \m
\end{eqnarray*}
...
\begin{eqnarray*}
\z = (z_1, \dots, z_k) = \Sigma (\K \hat{\beta} - \m) \\
...
is equal to the $t$ statistics for \Robject{speed}. The multiplicity-adjusted
