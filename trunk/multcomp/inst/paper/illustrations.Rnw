...
their covariance matrix $\S_n(\vartheta)$ which can be inspected
...
allele lengths. A sandwich estimator $\S_n(\theta)$ 
...
plot(ci1, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (ordinary ", bold(S)[n](theta), ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
plot(ci2, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (sandwich ", bold(S)[n](theta), ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
@
...
@
Again, a sandwich estimator of the covariance matrix $\S_n(\theta)$ can
