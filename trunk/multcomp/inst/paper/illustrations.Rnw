...
$n = 58$), and long ($10-12$, $n = 15$). Here, we are interested in comparing
...
A simple one-way ANOVA model is fitted to the data and $\K$
corresponding to Tukey all-pair comparisons is set up
...
about estimated parameters, their covariance matrix and
the linear functions defined under test. The \Rcmd{summary}
...
is not difference between any combination of the three
allele lengths. A sandwich estimator $\hat{\Sigma}$ 
might be more appropriate in this situation, and
the \Rarg{vcov} argument of \Rcmd{glht} can be used
to specify a function to compute some alternative 
estimator $\hat{\Sigma}$ as follows:
...
\begin{figure}
\begin{center}
<<alpha-confint-plot, echo = FALSE, fig = TRUE, width = 9, height = 4>>=
par(mai = par("mai") * c(1, 2.2, 1, 1))
layout(matrix(1:2, ncol = 2))
ci1 <- confint(glht(amod, linfct = mcp(alength = "Tukey")))
ci2 <- confint(glht(amod, linfct = mcp(alength = "Tukey"), vcov = sandwich))
plot(ci1, xlim = c(-0.6, 2.6), main = "Tukey", xlab = "Difference")
plot(ci2, xlim = c(-0.6, 2.6), main = "Tukey with Sandwich", xlab = "Difference")
@
...
\paragraph{Prediction of total body fat.}
...
Here, we fit the saturated model to the data and use the maximum-test
...
summary(bmod <- lm(DEXfat ~ ., data = bodyfat))
@
The linear function $\K$ is basically the identity matrix, except 
...
K <- cbind(0, diag(length(coef(bmod)) - 1))
rownames(K) <- names(coef(bmod))[-1]
bmod_glht <- glht(bmod, linfct = K)
@
...
summary(bmod_glht, test = Ftest())
@
but the source of the deviation from the global null-hypothesis
can only be inspected by the corresponding maximum-test, i.e., 
via
<<bodyfat-lm-maxtest, echo = TRUE>>=
summary(bmod_glht)
@
Only two covariates, waist and hip circumference, seem to be important
and caused the rejected of $H_0$. Alternatively, an MM-estimator
\citep{Yohai1987} can be used to fit a robust version of the above linear
model, the results coincde rather nicely:
...
second step, how a potential association can be described \citep[see][for
...
on the probability scale for the different `risk groups' are interesing 
and can be computed as follows. For each combination of gender and 
smoking behaviour, the probability of suffering from Alzheimer's
disease can be computed by computing the logit function 
...
we can set up a linear function $\K$ such that $\left(1 + \exp(- \K \hat{\vartheta})\right)^{-1}$
is the vector of estimated probabilities with simultaneous confidence intervals
\begin{eqnarray*}
\left(\left(1 + \exp\left(- \left(\K \hat{\vartheta} - q_\alpha \text{diag}\left(\hat{\S}^{-1/2}\right)\right)\right) \right)^{-1},
\left(1 + \exp\left(- \left(\K \hat{\vartheta} + q_\alpha \text{diag}\left(\hat{\S}^{-1/2}\right)\right)\right)\right)^{-1}\right)
\end{eqnarray*}
...
For our model, $\K$ is given by a matrix
\small
<<alzheimer-K, echo = TRUE>>=
K
@
\normalsize
and can easily be used to compute the above confidence intervals
<<alzheimer-probci, echo = TRUE>>=
gmod_ci <- confint(glht(gmod, linfct = K))
gmod_ci$confint <- apply(gmod_ci$confint, 2, binomial()$linkinv)
plot(gmod_ci, xlab = "Probability", xlim = c(0, 1))
@
The simultaneous confidence intervals are depicted in 
Figure~\ref{alzheimer-plot}. Using this representation of the results,
it is obvious that Alzheimer's disease is less frequent in non-smoking and
heavy smoking men compared to all other configurations.

\begin{figure}
\begin{center}
<<alzheimer-plot, echo = FALSE, fig = TRUE, mai = TRUE, width = 6, height = 4>>=
plot(gmod_ci, xlab = "Probability", xlim = c(0, 1), plot = "")
@
...
\paragraph{Acute Myeloid Leukemia. \label{AML}}
...
as their interactions with the patient's gender. Tukey all-pair comparisons
highlight that there seems to be a difference between `low' and `hight' scores
but that a futher `intermediate' score might be less important.
<<bullinger-survreg, echo = TRUE>>=
bmod <- survreg(Surv(time, event) ~ Sex * (Age + WBC + LDH + risk), 
                data = clinical)
