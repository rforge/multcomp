...
group differences (Tukey's all-pair differences):
...
@
and the general result reported above holds under very mild 
model assumptions.
...
@
Again, a sandwich estimator of the covariance matrix $\hat{\Sigma}$ can
