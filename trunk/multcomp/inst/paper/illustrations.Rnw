...
define $\K$ such that $\K \vartheta$ contains all three 
group differences:
...
about the estimated linear function $\K \hat{\vartheta}$ and 
their covariance matrix $\hat{\S}$ which can be inspected
...
\cite{Zeileis2004, Zeileis2006} which provides us with a 
heteroskedasticity-consistent estimator of the covariance matrix.
...
plot(ci1, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (ordinary ", hat(Sigma), ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
plot(ci2, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (sandwich ", hat(Sigma), ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
@
...
data("alzheimer", package = "coin")
y <- factor(alzheimer$disease == "Alzheimer's", 
...
smoking behaviour from both genders)
\small
...
plot(gmod_ci, xlab = "Probability", xlim = c(0, 1), main = "", ylim = c(0.5, 8.5))
@
...
as their interactions with the patient's gender. Tukey's all-pair comparisons
...
bmod <- survreg(Surv(time, event) ~ Sex * (Age + WBC + LDH + FLT3 + risk), 
                data = clinical)
