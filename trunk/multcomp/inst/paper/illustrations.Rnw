...
<<trees, echo = TRUE>>=
trees513 <- subset(trees513, !species %in% c("Tanne", "sonst. Nadelh."))
trees513$species <- trees513$species[,drop = TRUE]
pmod <- lmer(damage ~ species - 1 + (1 | lattice + plot), data = trees513,
                 family = binomial())
fe <- fixef(pmod)
K <- diag(length(fe))
colnames(K) <- rownames(K) <- names(fe)
K <- K[grep("species", names(fe)),]
myglht <- glht(pmod, K)
linv <- binomial()$linkinv
print(1 - linv(confint(myglht)$confint))
@
