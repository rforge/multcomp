...
have an advantage that corresponding simultaneous confidence are easily
...
elemental parameters together with consistent estimate of their covariance matrix. In
...
\Sraw$ are accessible in \RR{} with \Rcmd{coef()} and \Rcmd{vcov()}
...
plot(ci1, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (ordinary ", bold(S)[n], ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
plot(ci2, xlim = c(-0.6, 2.6), main = expression(paste("Tukey HSD (sandwich ", bold(S)[n], ")")), 
    xlab = "Difference", ylim = c(0.5, 3.5))
@
...
we can set up a linear function $\K$ such that $\left(1 + \exp(- \hat{\vartheta}_n)\right)^{-1}$
...
mmod <- lmer(damage ~ species - 1 + (1 | plot) + (1 | lattice), 
...
at risk and, as a consequence, the number of roe deers to be harvested in the
