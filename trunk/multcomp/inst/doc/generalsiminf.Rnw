...
Biometrical Journal, Volume 50, Number 3.                                      
...
set.seed(290875)
options(prompt = "R> ")
options(SweaveHooks = list(mai1 = function() par(mai = par("mai") * c(1, 2, 1, 1)),
                           mai2 = function() par(mai = par("mai") * c(1, 2.1, 1, 0.5)),
...
if (!file.exists("AML_Bullinger.rda")) {
    load(myurl <- url("http://www.stat.uni-muenchen.de/~hothorn/data/AML_Bullinger.rda", open = "r"))
    close(myurl)
}
...
\begin{figure}[hb!]
\begin{center}
<<alpha-confint-plot, echo = FALSE, fig = TRUE, width = 8, height = 3.5, mai2 = TRUE>>=
...
\begin{figure}[hb!]
\begin{center}
<<alzheimer-plot, echo = FALSE, fig = TRUE, width = 7, height = 5, mai1 = TRUE>>=
plot(gmod_ci, xlab = "Probability of Developing Alzheimer", 
...
\begin{figure}[t]
\begin{center}
<<trees-plot, echo = FALSE, mai3 = TRUE, fig = TRUE, width = 7, height = 5>>=
